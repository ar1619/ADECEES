...
import dataset
import evaluation
from GaussianDiffusion import GaussianDiffusionModel, get_beta_schedule, generate_simplex_noise
from helpers import *
...
def number_dates_between(start_date, end_date, average=False):
    """
    Get number of dates between start and end date
    :param start_date: start date in format YYYYMMDD
    :param end_date: end date in format YYYYMMDD
    :return: number of dates between start and end date
    """
    file_list = []
...
def train(training_dataset_loader, testing_dataset_loader, args, resume):
    ...
    iters = number_dates_between(args['start_date_train'], args['end_date_train'], average=args['average'])
    ...
    for epoch in tqdm_epoch:
        ...
        if epoch % 500 == 0 and epoch >= 0:
            ...
            print("500 Epochs took ", time.time() - start_time, " seconds")
            start_time = time.time()
...
def training_outputs(diffusion, x, est, noisy, epoch, row_size, ema, args, save_imgs=False, save_vids=False):
    ...
    if save_imgs:
        if epoch % 100 == 0:
            ...
            t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=x.device)
            noise = generate_simplex_noise(simplex, x, t, False, in_channels=args["channels"])
            x_t = diffusion.sample_q(x, t, noise)
            temp = diffusion.sample_p(ema, x_t, t)

            # array_to_save = x.detach().cpu().numpy()
            # np.save(f'./diffusion-training-images/ARGS={args["arg_num"]}/x_0-{epoch}epoch.npy', array_to_save)
...
def main():
    ...
    if args["dataset"].lower() == "cifar":
        ...
        training_dataset_loader = dataset.init_dataset_loader(training_dataset, args)
        testing_dataset_loader = dataset.init_dataset_loader(testing_dataset, args)
    elif args["dataset"].lower() == "oco2":
        training_dataset, testing_dataset = dataset.OCO2Dataset(args, DATA_DIR, True), \
                                                            dataset.OCO2Dataset(args, DATA_DIR, False)
        training_dataset_loader = dataset.cycle(training_dataset)
        testing_dataset_loader = dataset.cycle(testing_dataset)
    elif args["dataset"].lower() == "oco2sr":
        training_dataset, testing_dataset = dataset.OCO2Dataset_SR(args, DATA_DIR, True), \
                                            dataset.OCO2Dataset_SR(args, DATA_DIR, False)
        training_dataset_loader = dataset.cycle(training_dataset)
        testing_dataset_loader = dataset.cycle(testing_dataset)
        print("Validation dataset days:", training_dataset.val_filenames)
    else:
        # load NFBS dataset
