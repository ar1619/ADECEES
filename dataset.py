...
def dates_between(start_date, end_date, average=False):
    """
    ...
    :return: list of filenames between start and end date
    """
    file_list = []
    ...
    while current_date <= end_date:
        current_date_file = current_date.strftime('%Y%m%d')
        file_list.append(current_date_file)
        current_date += timedelta(days=1)
    # If average is true, keep one day every seven day
    if average:
...
def normalize_tensor(tensor):
    ...
    for i in range(tensor.shape[0]):
        # First, find the minimum and maximum values in the channel
        ...
        normalized_channel = (tensor[i] - min_val) / range_val
        
        # Scale to [-1, 1] and store in the result tensor
        scaled_tensor[i] = normalized_channel * 2 - 1
    
    return scaled_tensor

def get_a_week(start_date, start_day=0):
    start_date = datetime.strptime(start_date, '%Y%m%d')
    if start_date.weekday() < start_day:
        start_date = start_date + timedelta(days=start_day - start_date.weekday())
    elif start_date.weekday() > start_day:
        difference = start_date.weekday() - start_day
        start_date = start_date + timedelta(days=7 - difference)
    end_date = start_date + timedelta(days=6)
    return start_date.strftime('%Y%m%d'), end_date.strftime('%Y%m%d')

def normalize_subarray(array):
    vmax = np.amax(array)
    vmin = np.min(np.where(array >= 0, array, np.inf))
    range_data = vmax - vmin

    normalized_data = (array - vmin)/range_data

    return normalized_data

# def get_sample(data_paths, coordinates, data_dir, size=512, normalize=False):
    # get 

def get_average(data_paths, coordinates, data_dir, start_date, start_day=0, size=512, normalize=False, channels=1, hdf5=False):
    start_date, end_date = get_a_week(start_date, start_day)
    date_list = dates_between(start_date, end_date)
    to_average = np.zeros((size, size))
    for date in date_list:
        if hdf5:
            filename = data_dir+'oco2_GEOS_L3CO2_day_{}_B10206Ar.nc4'.format(date)
            original_array = xr.open_dataset(filename)
            original_array = array_centered_on_point(original_array, coordinates).astype(np.float32)[0]
        else:
            start_x = 512//2 - size//2
            start_y = 512//2 - size//2
            filename = data_dir+'{}.npy'.format(date)
            original_array = np.load(filename)
            original_array = original_array[start_x:start_x+size, start_y:start_y+size]
        if normalize:
            original_array = normalize_subarray(original_array)
        to_average += original_array
    if channels > 1:
        to_average = add_additional_datasets(to_average, coordinates, data_paths, date_list)
    return to_average

def get_slice(array, size):
    x_start = 512//2 - size//2
    y_start = 512//2 - size//2
    return array[x_start:x_start+size, y_start:y_start+size]
...
class OCO2Dataset(Dataset):
    """CO2 dataset pre and post factory."""
    def __init__(self, args, DATA_DIR, closed_factory=False, transform=None, img_size=(32, 32)):
        """
        Args:
        ...
        self.start_date_train = args["start_date_train"]
        self.end_date_train = args["end_date_train"]
        self.start_date_test = args["start_date_test"]
        self.end_date_test = args["end_date_test"]
        self.coordinates = args["coordinates"]
        self.average = args["average"]
        self.closed_factory = closed_factory
        if closed_factory:
            self.filenames = dates_between(self.start_date_train, self.end_date_train, self.average)
        else:
            self.filenames = dates_between(self.start_date_test, self.end_date_test, self.average)
        if self.average:
    ...
    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        if self.average:
            data_dir = self.DATA_DIR+'OCO-2/'
            array = get_average(data_dir, self.filenames[idx], 0, 32, False, True, self.coordinates)
            array = np.expand_dims(array,axis=0).astype(np.float32)
        else:
            filename = self.DATA_DIR+'OCO-2/oco2_GEOS_L3CO2_day_{}_B10206Ar.nc4'.format(self.filenames[idx])
            original_array = xr.open_dataset(filename)
            array = array_centered_on_point(original_array, self.coordinates).astype(np.float32)
        array = np.expand_dims(array[0],axis=-1)
...
class OCO2Dataset_SR(Dataset):
    """CO2 dataset pre and post factory."""
    def __init__(self, args, DATA_DIR, closed_factory=False, transform=True, img_size=(512, 512)):
        """
        Args:
            DATA_DIR (string): Directory with all the images.
            transform (callable, optional): Optional transform to be applied
        ...
        if closed_factory:
            self.filenames = dates_between(self.start_date_train, self.end_date_train, self.average)
            self.filenames, self.val_filenames = split_data_loader(self.filenames)
        else:
            self.filenames = dates_between(self.start_date_test, self.end_date_test, self.average)
        if self.average:
            self.filenames = self.filenames[:-1]
    ...
    def __getitem__(self, idx):
        ...
        if torch.is_tensor(idx):
            idx = idx.tolist()
        data_dir = self.DATA_DIR+self.mine+'/'
        if self.average:
            array = get_average(self.data_paths, self.coordinates, data_dir, self.filenames[idx], 2, self.size, True, self.channels)
            array = np.expand_dims(array,axis=0)
        else:
            filename = data_dir+'{}.npy'.format(self.filenames[idx])
            array = np.load(filename)
            array = get_slice(array, self.size)
            array = np.expand_dims(array,axis=0)
