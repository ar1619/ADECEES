...
def segment_values_above_threshold(image, threshold_plus=0.5, threshold_minus=-0.5, segmentation='positive'):
    ...
    return mask
def diff_heatmap(real: torch.Tensor, recon: torch.Tensor, filename, save=True):
    print(len(recon))
    diff = real - recon
    diff = diff.cpu().numpy()
    real = real.cpu().numpy()
    recon = recon.cpu().numpy()
    filename_diff = 'diff_heatmaps/diff/' + filename
    np.save(filename_diff, diff)
    if save:
        fig = plt.figure(figsize=(10, 8))
        gs = gridspec.GridSpec(2, 2, height_ratios=[1, 1.5], width_ratios=[1, 1])
        ax1 = fig.add_subplot(gs[0, 0])
        im1 = ax1.imshow(real[0, 0, ...], aspect='equal')
        cbar1 = plt.colorbar(im1, ax=ax1, fraction=0.05)
        ax1.set_title('Real')
        ax1.axis('off')

        ax2 = fig.add_subplot(gs[0, 1])
        im2 = ax2.imshow(recon[0, 0, ...], aspect='equal')
        cbar2 = plt.colorbar(im2, ax=ax2, fraction=0.05)
        ax2.set_title('Recon')
        ax2.axis('off')

        ax3 = fig.add_subplot(gs[1, :])
        im3 = ax3.imshow(diff[0, 0, ...], aspect='equal', vmax=1, vmin=-1)
        cbar = plt.colorbar(im3, ax=ax3, orientation='horizontal', fraction=0.05)
        ax3.set_title('Diff')
        ax3.axis('off')

        # ax4 = fig.add_subplot(gs[1, 1])
        # im4 = ax4.imshow(segment_values_above_threshold(diff[0, 0, ...]), aspect='equal', cmap='seismic')
        # ax4.set_title('Areas of High Difference')
        # ax4.axis('off')

        filename = 'diff_heatmaps/' + filename
...
def plot_sequence(diffs, segmentation_type, args_n):
    ...
    plt.savefig(filename)
    plt.clf()
def plot_ensemble(image, mean_output, variance_output, filename, n_ensemble=10, segmentation_type='positive'):
    """
    ...
    im3 = ax3.imshow(segment_values_above_threshold(diff[0, 0, ...], segmentation=segmentation_type), aspect='equal')
    # cbar3 = plt.colorbar(im3, ax=ax3, fraction=0.05)
    # ax3.set_title('Segmented anomalies')
    ax3.axis('off')

    ax4 = fig.add_subplot(gs[3])
    im4 = ax4.imshow(variance_output[0, 0, ...], aspect='equal', cmap='inferno', vmax=0.922, vmin=0.00023)
...
def main():
    ...
    for i in range(dataset_size):
        new = next(loader)
        if i % 2 == 0:
            input = torch.unsqueeze(new["image"],0).to(device)
            ...
            if ensemble:
                ...
                inputs.append(input)
                # plot_ensemble(input, mean_output, variance_output, "{}_{}_{}_{}.png".format(filename, args['arg_num'], str(n_ensemble), segmentation_type), n_ensemble, segmentation_type)
            else:
                output = diffusion.forward_backward(
                        unet, input,
                        see_whole_sequence=sequence,
                        t_distance=200
                        )
                # diff_heatmap(input, output, "{}_{}.png".format(filename, args['arg_num'], save=False))
                whole_sequence = [x.cpu().numpy() for x in output]
                np.save('whole_sequence/{}_{}.npy'.format(filename, args['arg_num']), whole_sequence)
        # if mean_outputs != []:
            # diffs = torch.stack(inputs) - torch.stack(mean_outputs)
            diff = input - mean_output
            diff = diff.cpu().numpy()

            # save input, mean output, diffs as numpy arrays
            np.save('wind/inputs{}_{}.npy'.format(args['arg_num'], filename), input.cpu().numpy())
            np.save('wind/mean_outputs{}_{}.npy'.format(args['arg_num'], filename), mean_output.cpu().numpy())
            np.save('wind/diffs{}_{}.npy'.format(args['arg_num'], filename), diff)
        # diffs = np.array(diffs[0:6])
        
        # plot_sequence(diffs, segmentation_type, args['arg_num'])
